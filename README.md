...
Project-authoritative captions support review notes plus an append-only `draft → review → approved` status history with actor and timestamp. Translation clones and imported cues start as draft; preview burn-in, WebVTT and production capture share an approved-only gate. Notes and transitions survive project/package persistence, while duplicate note IDs and malformed audit entries are rejected. Assignment, threaded comments, translation memory, TTML and IMSC delivery remain explicit commercial-product gaps.
