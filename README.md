...
Project-authoritative captions support assigned reviewers, review notes plus an append-only `draft → review → approved` status history with actor and timestamp. When assigned, only that reviewer may approve a cue. Translation clones and imported cues start as draft; preview burn-in, WebVTT and production capture share an approved-only gate. Notes and transitions survive project/package persistence, while duplicate note IDs and malformed audit entries are rejected. Threaded comments, translation memory, TTML and IMSC delivery remain explicit commercial-product gaps.
