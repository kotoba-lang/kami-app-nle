...
Project-authoritative captions support language-track cloning, per-cue deletion, validated WebVTT import, BCP-47-style language tags and active burn-in/export language selection. Cloning copies timing, source text and style into deterministic target-language cue IDs, replacing an older target draft while preserving other languages; translators can then edit or delete cues in place. Import retains the same language isolation and stable frame-rounded export. Translation memory, review status, rich typography, TTML and IMSC delivery remain explicit commercial-product gaps.
