...
Project-authoritative captions support validated WebVTT import, BCP-47-style language tags, active burn-in/export language selection, multiline text, top/bottom position, left/center/right alignment and bounded font scale inside a 5% title-safe area. Import replaces only its target language and preserves every other language; deterministic cue IDs and frame-rounded timing make subsequent WebVTT export stable. The selected language is rendered onto the decoded preview canvas, so production capture contains the same localized burn-in. Translation memory, rich typography, TTML and IMSC delivery remain explicit commercial-product gaps.
