...
Project-authoritative captions support `draft → review → approved` status, language-track cloning, per-cue deletion, validated WebVTT import and active language selection. Translation clones and imported cues start as draft. Preview burn-in, WebVTT and production capture share an approved-only delivery gate; legacy and manually-authored cues retain approved compatibility. Translation memory, assignment/comments, rich typography, TTML and IMSC delivery remain explicit commercial-product gaps.
